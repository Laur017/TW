  ...
  form.addEventListener("submit", (e) => {
    ...
    sendMessage(msgToSend.value);
    
  });

  setInterval( function(){  checkMsg(); },1000);
...
function sendMessage(theMsg) {
  ...
  myData.username = inputVal.value;
...
function getResponse(respMsg) {
  //var response = "Hello from ChatBot, " + username.value + "!";

  var item = document.getElementById("test");
  var textBox = document.createElement("div");
  textBox.textContent = respMsg;
...
function checkMsg()
{
    ...
    var request = new XMLHttpRequest();
    var apiResp;
    justUrl = "http://localhost:5000/api/messages?email="+localStorage.getItem("email");
    
    request.onreadystatechange = function()
    {
        //console.log("My name: " + myData.username)
        ...
        apiResp.forEach(element => {
            if(element.fromUserId != myData.id){
            if(!MsgIGot.some(elem => elem === element.id))
            {
                console.log(element.content );
                MsgIGot.push(element.id);
                getResponse(element.content)
            }
        }
        });
        console.log(MsgIGot)
